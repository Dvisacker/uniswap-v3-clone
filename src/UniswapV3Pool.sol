...
contract UniswapV3Pool {
    ...
    function swap(
        ...
        if (
            zeroForOne
                ? (sqrtPriceLimitX96 > slot0.sqrtPriceX96 || sqrtPriceLimitX96 < TickMath.MIN_SQRT_PRICE)
                : (sqrtPriceLimitX96 < slot0.sqrtPriceX96 || sqrtPriceLimitX96 > TickMath.MAX_SQRT_PRICE)
        ) {
        ...
        while (state.amountSpecifiedRemaining > 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
                if (step.initialized) {
                    ...
                    state.liquidityDelta = LiquidityMath.addLiquidity(state.liquidityDelta, liquidityDelta);
